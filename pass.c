...
void pass_analyzer() {
    ...
    sprintf(report + strlen(report), "Length: %d\nIncludes: ", var.len); //appends length of string to report
...
void pass_generator() {
    int length;
    char charset[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%%^&*"; //% twice
    char password[100];
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 4:
                ...
                return; //cannot return 0 as it is void
            default:
